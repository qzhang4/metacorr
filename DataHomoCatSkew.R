## data generation: true dist=skewed, homogeneous ##
library(MASS)
lp=1000
ssize=200
NS=50
...
datadir=paste0('/gpfs/home/qzhang4/metacorr/Homogeneous/DataHomoCatSkew/cond',cond.idx+1000,'/')
#system(paste0('rm -r ',datadir))
dir.create(datadir)

# obtain the approximated population distribution for universal cutoff values;
popN=1000000 # sample size=1 million to appoximate the population counterpart;
msmterrxPop=msmterryPop=ixPop=iyPop=matrix(NA,popN,I)
z1uPop=rchisq(popN,1)
z1Pop=(z1uPop-1)/sqrt(2)
z2uPop=rchisq(popN,1)
z2Pop=(z2uPop-1)/sqrt(2)
xtPop=z1Pop
ytPop=rho*z1Pop+sqrt(1-rho^2)*z2Pop
msmterrxPop[,1:I] <- mvrnorm(popN,mumsmterrx,sigmamsmterrx) # generate msmt error for x;
msmterryPop[,1:I] <- mvrnorm(popN,mumsmterry,sigmamsmterry) # generate msmt error for y;
ixPop[,1:I]=xtPop%*%t(as.matrix(faloading))+msmterrxPop[,1:I] # item scores for x;
iyPop[,1:I]=ytPop%*%t(as.matrix(faloading))+msmterryPop[,1:I] # item scores for y;
...
for (i in 1:lp) {
  ...
  nn=1
  while (nn <= NS) {
    # obtain obtain the sample data and apply cutoff values from the population;
    ...
    if (cat==2) {
      if (dist==1) {
        for (j in 1:I) {
          # the cutoff based on approximate population distribution
          cutoffx <- quantile(ixPop[,j],.5)
          cutoffy <- quantile(iyPop[,j],.5)
          xidx1 <- which(ix[,j]<cutoffx)
          xidx2 <- which(ix[,j]>=cutoffx)
          yidx1 <- which(iy[,j]<cutoffy)
          yidx2 <- which(iy[,j]>=cutoffy)
          ix[xidx1,j] <- 1
          ix[xidx2,j] <- 2
          iy[yidx1,j] <- 1
          iy[yidx2,j] <- 2
      ...
      if (dist==2) {
        for (j in 1:I) {
          cutoffx <- quantile(ixPop[,j],.8)
          cutoffy <- quantile(iyPop[,j],.8)
          xidx1 <- which(ix[,j]<cutoffx)
          xidx2 <- which(ix[,j]>=cutoffx)
          yidx1 <- which(iy[,j]<cutoffy)
          yidx2 <- which(iy[,j]>=cutoffy)
      ...
      if (dist==4) {
        ...
        for (j in (I/2+1):I) {
          cutoffx <- quantile(ixPop[,j],.2)
          cutoffy <- quantile(iyPop[,j],.2)
          xidx1 <- which(ix[,j]<cutoffx)
          xidx2 <- which(ix[,j]>=cutoffx)
          yidx1 <- which(iy[,j]<cutoffy)
          yidx2 <- which(iy[,j]>=cutoffy)
    ...
    if (cat==5) {
      if (dist==1) {
        for (j in 1:I) {
          cutoffx <- quantile(ixPop[,j],c(.1,.3,.7,.9))
          cutoffy <- quantile(iyPop[,j],c(.1,.3,.7,.9))
          xidx1 <- which(ix[,j]<cutoffx[1])
          xidx2 <- which((ix[,j]>=cutoffx[1])&(ix[,j]<cutoffx[2]))
          xidx3 <- which((ix[,j]>=cutoffx[2])&(ix[,j]<cutoffx[3]))
          xidx4 <- which((ix[,j]>=cutoffx[3])&(ix[,j]<cutoffx[4]))
          xidx5 <- which(ix[,j]>=cutoffx[4])
          yidx1 <- which(iy[,j]<cutoffy[1])
          yidx2 <- which((iy[,j]>=cutoffy[1])&(iy[,j]<cutoffy[2]))
          yidx3 <- which((iy[,j]>=cutoffy[2])&(iy[,j]<cutoffy[3]))
          yidx4 <- which((iy[,j]>=cutoffy[3])&(iy[,j]<cutoffy[4]))
          yidx5 <- which(iy[,j]>=cutoffy[4])
          ix[xidx1,j] <- 1
          ix[xidx2,j] <- 2
          ix[xidx3,j] <- 3
          ix[xidx4,j] <- 4
      ...
      if (dist==2) {
        for (j in 1:I) {
          cutoffx <- quantile(ixPop[,j],c(.4,.65,.85,.95))
          cutoffy <- quantile(iyPop[,j],c(.4,.65,.85,.95))
          xidx1 <- which(ix[,j]<cutoffx[1])
          xidx2 <- which((ix[,j]>=cutoffx[1])&(ix[,j]<cutoffx[2]))
          xidx3 <- which((ix[,j]>=cutoffx[2])&(ix[,j]<cutoffx[3]))
          xidx4 <- which((ix[,j]>=cutoffx[3])&(ix[,j]<cutoffx[4]))
      ...
      if (dist==4) {
        ...
        for (j in (I/2+1):I) {
          cutoffx <- quantile(ixPop[,j],c(.05,.15,.35,.6))
          cutoffy <- quantile(iyPop[,j],c(.05,.15,.35,.6))
          xidx1 <- which(ix[,j]<cutoffx[1])
          xidx2 <- which((ix[,j]>=cutoffx[1])&(ix[,j]<cutoffx[2]))
          xidx3 <- which((ix[,j]>=cutoffx[2])&(ix[,j]<cutoffx[3]))
          xidx4 <- which((ix[,j]>=cutoffx[3])&(ix[,j]<cutoffx[4]))
