## data generation,continous items,true dist=norm, heterogeneous ##
library(MASS)
lp=1000
...
I1=c(4,8,12)
cond=expand.grid(zrho1,tau1,I1)
...
datadir=paste0('/gpfs/home/qzhang4/metacorr/Heterogeneous/DataHeteContNorm/cond',cond.idx+1000,'/')
for (sz in 1:nrow(Nn)) {
  ssize=Nn[sz,1]
  NS=Nn[sz,2]
  ...
  for (i in 1:lp) {
    ...
    while (nn <= NS) {
      ...
      xt=rnorm(N[nn])
      w2=rnorm(N[nn])
      yt=rhos[nn]*xt+sqrt(1-rhos[nn]^2)*w2
